...
class Paint:
    ...
    def draw(self, event):
        ...
        if self._tool in (LINE, RECTANGLE, OVAL):
            ...
            if self._tool == PENCIL:
                self.canvas.create_line(self.lastx, self.lasty, event.x, event.y, fill = self._color, capstyle=ROUND, smooth=True, splinesteps=36, width = self._width)
                ...
                self.canvas.create_oval(x1, y1, x2, y2, fill = self._color, outline = self._color)
            elif self._tool == BRUSH1:
                if self._width is None:
                    x1, y1 = (event.x - 5), (event.y - 5)
                    x2, y2 = (event.x + 5), (event.y + 5)
                else:
    ...
    def click(self, event):
        ...
        x, y = event.x, event.y
        if self._tool == LINE:
            self._obj = self.canvas.create_line((x, y, x, y), fill = self._color, width = self._width, capstyle=ROUND, smooth=True, splinesteps=36)
...
class Tool:
    def __init__(self, whiteboard, parent=None):
        ...
        #self.brush1 = PhotoImage(file="Images/brush_tool.gif")
...
#----------------------------------------------------------------------
    ...
    def save_file(self, event):
        filename = filedialog.asksaveasfilename(title = "Select file",filetypes = (("jpeg files","*.jpg"),("icon files","*.ico"),("wmf files","*.wmf"),("gif files","*.gif"),("bmp files","*.bmp"),("png files", "*.png")))
...
root.title("Laba3")
canvas = Canvas(highlightbackground='black', bg='white', width = 800, height = 600)
